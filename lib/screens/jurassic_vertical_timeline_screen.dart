// lib/screens/jurassic_vertical_timeline_screen.dart
...
class JurassicVerticalTimelineScreen extends StatelessWidget {
  final JurassicData data;
  final int userSteps;

  const JurassicVerticalTimelineScreen({
    super.key,
    required this.data,
    required this.userSteps,
  });

  static const Color _bottleGreen = Color(0xFF1B5E4A);
  ...
  Widget build(BuildContext context) {
    ...
    const double width = 300; // шире коридор
    const double horizontalScreenPadding = 8; // меньше отступ от краёв экрана
    ...
    for (var i = 0; i < nodes.length; i++) {
      ...
      final double scale = 1.0 + 0.2 * sin(i * pi / 4);
    ...
    return Scaffold(
      ...
      body: Stack(
        children: [
          Positioned.fill(
            child: Image.asset(
              'assets/images/jurassic/background.png',
              fit: BoxFit.cover,
            ),
          ),
          Positioned.fill(
  ...
  Widget _buildCircle(_DotPoint p) {
    ...
    final Color circleColor = isUnlocked ? _bottleGreen : Colors.grey.shade400;
  ...
  // текст снова слева/справа от круга, ширина считается от центра до края
  Widget _buildLabels(_DotPoint p, {required double width}) {
    ...
    if (textOnLeft) {
      ...
      return Positioned(
        top: top,
        left: 0,
        right: width - (p.xCenter + circleRadius), // оставляем место под круг
        child: Row(
          crossAxisAlignment: CrossAxisAlignment.start,
          children: [
            Expanded(
              // текст занимает всё, что осталось слева
              child: Padding(
                padding: const EdgeInsets.only(right: gap),
                child: textColumn,
              ),
            ),
            SizedBox(width: p.size), // «пустое» место под круг
          ],
        ),
      );
    } else {
      ...
      return Positioned(
        ...
        child: Row(
          crossAxisAlignment: CrossAxisAlignment.start,
          children: [
            SizedBox(width: p.size), // место под круг слева
            SizedBox(width: gap),
            Expanded(
              // текст занимает всё, что осталось справа
              child: textColumn,
            ),
          ],
        ),
